...
### Secure copy (scp) this file to your qbio_490_name repository. DO NOT EDIT IN sp26_course_materials! ###
...
colnames(mtcars)
rownames(mtcars)
target_rows <- c("Duster 360", "Lincoln Continental", "Maserati Bora")
mtcars[target_rows, "hp"]
```
...
```{r}
write.csv(clinic, "/home1/kwang895/490_cluster/analysis_data/brca_clinical_data.csv", row.names=FALSE)
```

To read the csv back into your environment as a dataframe:

```{r}
clinic <- read.csv("/home1/kwang895/490_cluster/analysis_data/brca_clinical_data.csv")
```
...
The data type of $ gender is "chr".
...
The age is 55 + 3767/365, which is approximately 65.
...
clinic$age_at_diagnosis[1]
clinic$last_contact_days_to[1]
```
...
```{r Exercise 4.1}
# create plot here
plot(clinic$age_at_diagnosis,
     clinic$last_contact_days_to,
     main = "Age at Diagnosis vs. Days to Last Contact",
...
```{r Exercise 4.2}
# create plot here
boxplot(clinic$age_at_diagnosis ~ clinic$vital_status,
        main = "Age at Diagnosis by Vital Status",
        xlab = "Vital Status",
...
```{r}
under30_over60_mask <- under30_over60_mask <- clinic$age_at_diagnosis < 35 | clinic$age_at_diagnosis > 60
```
...
```{r}
no_middle_clinic <- no_middle_clinic <- clinic[under30_over60_mask, ]
```
...
The y-axis represents the survival probability (proportion of patients still alive) over time.
# 2. What does the x axis represent?
The x-axis represents time in days since diagnosis.
# 3. What age group appears to have the greatest rate of survival?
The "Middle" age group (35–50) appears to have the greatest survival rate, as the red curve stays the highest over time, plateauing around ~0.50–0.52 at long follow-up.
# 4. What age group appears to have the worst rate of survival? 
The Old age group (≥50) has the worst rate of survival. Although the Young (blue) curve drops sharply to 0 around day ~3500, that reflects very few patients remaining (small sample size), making it unreliable. The Old (green) curve consistently declines and levels off at approximately 0.30, the lowest of all groups at long-term follow-up.
# 5. What is the approx. 15-year survival rate for "Old" breast cancer patients?
Approximately 30% (read from the curve at ~5475 days on the x-axis).
...
The p-value is p = 0.2. This is not statistically significant (p > 0.05). This means we cannot conclude that there is a statistically meaningful difference in survival rates between the three age groups in this dataset. The observed differences in the curves may be due to chance, small sample sizes (especially in the Young group), or insufficient follow-up time rather than a true biological effect of age on breast cancer survival.


###### (6) Submitting Coding HW

Your homework should be a completed secure copy (scp) of this file (original file in sp26_course_materials). Push this downloaded file to GitHub through your qbio_490_name local repository and submit a link to your GitHub repository to the Introduction To Clinical HW Brightspace Assignment.  